# revise typeclassifier.py to match viewclassifier.py style
# 3 to 2 classes (single organ, whole plant)
...
IMG_SIZE = 224
BATCH_SIZE = 16 # total image = 170, so batch count is 6
EPOCHS = 20
LR = 0.0003
WEIGHT_DECAY = 0.05
NUM_CLASSES = 2  # single organ / whole plant
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"


class ViewClassifier(nn.Module):
    """
    MobileNet-v3 based classifier for 2 classes.
...
def train(
    train_dir: str,
    val_dir: str,
    out_path: str = "viewclassifier_best.pt",
    epochs: int = EPOCHS,
    lr: float = LR,
    ...
    model = ViewClassifier(num_classes=num_classes, pretrained=True)
...
def load_model(checkpoint_path: str, device: str = DEVICE, num_classes: int = NUM_CLASSES) -> nn.Module:
    model = ViewClassifier(num_classes=num_classes, pretrained=False)
...
if __name__ == "__main__":    
    ...
    TRAIN_DIR = "D:\\AIHub\\dataset\\ViewClassifier\\Train"
    VAL_DIR = "D:\\AIHub\\dataset\\ViewClassifier\\Val"
