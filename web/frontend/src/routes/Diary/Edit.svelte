...
import EditTop from '../../lib/EditTop.svelte'
import EditUI from '../../lib/EditUI.svelte';
import EditUIVal from '../../lib/EditUIVal.svelte';
...
function get_diary(){
  ...
  /* success_callback */
    (response) => {
        ...
        cookieStore.set({ name: 'plant_id', value: plant_id, path: '/'});
        get_plant();
    },
  /* failure_callback */
    (error) => {
...
<!-- This contains apply changes button -->
<EditTop plantid={plant_id} /> 
...
<main>
  ...
  <MenuTitle title="Edit Diary" icon={logo_diary} />

  <div class="info center">
      <h3 style="color:green">🥒Plant name: <input id="plant_name" class="title" value={plant["plant_name"]}/></h3>
      <h4>🌱Growth since <b>{plant["start_date"]}, {diffDays} Day(s)</b></h4>
      <EditUIVal title="Comment" description={diary["note"]} color="transparent" wide={true} comment={true}/>
  </div>
  ...
  <div class="wrapper">
    <div class="side">
      ...
      <div class="ResultSet">
        <EditUI title="Unit" description={diary["unit"]} color="blue"/>
        <EditUI title="Type" description={diary["organ_type"]} color="blue"/>
        <EditUI title="Stage" description={diary["stage"]} color={stagecolor}/>
      </div>  
       <ParaDeco color="white" title="Diagnosis" size="24" shadow="shadow" />
      <div class="ResultSet">
        <EditUI title="Health" description={diary["diagnosis"]} color={healthcolor}/>
        <EditUI title="Chlorophyll" description={diary["chlorophyll_content"]} color={chlorophyllcolor}/>
        <EditUIVal title="Greenness" description={diary["measurement"]} color={measurementcolor}/>
      </div>
    </div>
