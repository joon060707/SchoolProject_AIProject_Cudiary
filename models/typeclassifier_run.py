...
import typeclassifier
import torch.optim as optim
from torch.utils.data import DataLoader
from torchvision import transforms, datasets
from tqdm import tqdm

CLASS_NAMES = ['Leaf', 'Flower', 'Fruit']  # Example class names
...
def test_model(model: nn.Module, device: str = "cuda"):
    ...
    test_dir = "D:\\AIHub\\dataset\\TypeClassifier\\Test"  # path to test dataset
    batch_size = 32
    num_workers = 4
    pin_memory = True
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig("typeclassifier_confusion_matrix.png")
...
if __name__ == "__main__":
    ...
    import sys
    MODEL_PATH = "typeclassifier_best.pt"
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = typeclassifier.load_model(MODEL_PATH, device=device)
    model.eval()
